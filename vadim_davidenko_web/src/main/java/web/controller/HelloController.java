...
import web.service.EmployeeServiceImpl;
...
public class HelloController {
    ...
    public String employee(@RequestParam("login") String name, Model model) {
        Employee emp = service.getEmployee(Long.parseLong(name));
        model.addAttribute("name", emp.toString());
        return "index";
    }
...
//        employeeService.findAll();
    ...
    @RequestMapping(value = "/testAngular", method = RequestMethod.GET)
    public String angularTest() {
        return "testAngular";
    }

    @RequestMapping(value = "/testBody.html", method = RequestMethod.GET)
    public @ResponseBody String testBody(@RequestParam("name") String name) {
        Random random = new Random();
        int num = random.nextInt(10);
        return String.valueOf(num) + " " + name;
    }
}
